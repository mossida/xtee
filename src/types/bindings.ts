...
export type ActuatorStatus =
  | { status: "loading"; data: { target: number } }
  | { status: "keeping"; data: { target: number } }
  | { status: "tuning" }
  | { status: "idle" };
...
export type Event =
  | { type: "init" }
  | { type: "weight"; data: number }
  | { type: "motor-status"; data: [number, MotorStatus] }
  | { type: "actuator-status"; data: ActuatorStatus }
  | {
      type: "controller-status";
      data: { controller: Controller; status: ControllerStatus };
    };

export type MotorMovement = {
  speed: number;
  direction: boolean;
  rotations: number;
};
...
export type MotorStatus =
  | { status: "idle" }
  | { status: "stopping" }
  | { status: "spinning"; data: { position: number; remaining: number } };
